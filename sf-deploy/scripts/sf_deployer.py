#!/usr/bin/env python3
"""
sf_deployer.py — INTERNAL: run one `sf` CLI deployment of one package.

This is the pipeline's low-level executor, not an entry point: it knows
nothing about sheets, deltas, plans or translations. The public command is
`deploy.py`, which computes the plan and calls this module once per package
part. Other modules also talk to the org (org_snapshot, verify_deploy,
grant_fls, …); this one is simply where the `sf project deploy` call lives.

Safety model:
  * Default action is check-only validation (`sf project deploy validate`) —
    it NEVER writes to the org; `--start` performs the real deploy.
  * A deploy is refused unless the validation report shows zero ERRORs, unless
    `--skip-validation-gate` is explicitly passed.
  * Every non-zero exit auto-writes a failure record plus a DRAFT lesson
    (log_failure.py), so a failure can never pass silently.

Usage (normally invoked by deploy.py):
  python scripts/sf_deployer.py --package manifest/package.xml --target-org "ERP DEV 02"
  python scripts/sf_deployer.py --start --package manifest/package.xml --target-org "ERP DEV 02"
  python scripts/sf_deployer.py --start --pre-destructive manifest/destructiveChanges.xml \
      --package manifest/destructive_package.xml --target-org "ERP DEV 02"
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not org:
        ...
        print("   python scripts/sf_deployer.py --target-org 1            # by list index")
        print("   python scripts/sf_deployer.py --target-org \"ERP DEV 02\"  # by alias")
        if not orgs:
